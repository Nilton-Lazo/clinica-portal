...
export function useAreaJefatura() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getNextAreaJefaturaCodigo();
        if (!alive) return;
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      if (mode === "new") {
        await createAreaJefatura({ codigo: c, descripcion: d, estado });
        toast.success("Área o jefatura creada.");
        setPage(1);
        await refresh({ page: 1 });
        resetToNew();
      } else {
        const res = await updateAreaJefatura(selected!.id, { codigo: c, descripcion: d, estado });
        toast.success("Cambios guardados.");
        await refresh();
        loadForEdit(res.data);
      }
    } catch (e) {
      toast.error(getApiErrorMessage(e, "No se pudo guardar el área o jefatura."));
    } finally {
      setSaving(false);
    }
  ...
  const onDeactivateConfirmed = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await deactivateAreaJefatura(selected.id);
      setConfirmDeactivateOpen(false);
      toast.success("Área o jefatura desactivada.");
      await refresh();
      loadForEdit(res.data);
    } catch (e) {
      setConfirmDeactivateOpen(false);
      toast.error(getApiErrorMessage(e, "No se pudo desactivar el área o jefatura."));
    } finally {
      setSaving(false);
    }

...
import { useFormaPagoCaja } from "../forma-pago/hooks/useFormaPagoCaja";
...
export default function FormaPagoCajaPage() {
  const vm = useFormaPagoCaja();
  const isLgUp = useIsLgUp();
  const formRef = React.useRef<HTMLDivElement | null>(null);

  useFicherosRealtimeRefresh(vm, ["caja_forma_pago"]);
    ...
    <div className="flex w-full flex-col gap-4 lg:min-h-0 lg:flex-1 lg:overflow-hidden lg:gap-2">
      <div className="flex shrink-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="text-base font-semibold text-(--color-text-primary)">Forma de pago</div>

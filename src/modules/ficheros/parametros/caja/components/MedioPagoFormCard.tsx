...
import type { FormaPagoCajaOption, MedioPagoCajaItem } from "../services/medioPagoCaja.service";

type Mode = "new" | "edit";

export default function MedioPagoFormCard(props: {
  mode: Mode;
  selected: MedioPagoCajaItem | null;
  codigo: string;
  descripcion: string;
  ...
  formaPagoId: string;
  onFormaPagoIdChange: (v: string) => void;
  saving: boolean;
  isValid: boolean;
  isDirty: boolean;
  ...
  const formaPagoOptions: SelectOption[] = [
    ...
    ...formasPago.map((fp) => ({ value: String(fp.id), label: fp.descripcion })),
  ];
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-xs text-(--color-text-secondary)">
            {mode === "new" ? "Crea un medio de pago." : "Modifica campos y guarda cambios."}
          </div>
        </div>

...
export function useTopico() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getNextTopicoCodigo();
        if (!alive) return;
  ...
  const refresh = useCallback(async (next?: { page?: number; perPage?: number }) => {
    ...
    try {
      ...
      const msg = getApiErrorMessage(e, "No se pudo cargar la lista de tópicos.");
  ...
  const onSave = useCallback(async () => {
    ...
    if (mode === "edit" && !selected) { toast.error("Selecciona un tópico para editar."); return; }
    if (!isDirty) { toast.error("No hay cambios para guardar."); return; }
    if (saving) return;
    setSaving(true);
    try {
      if (mode === "new") {
        await createTopico({ codigo: c, descripcion: d, estado });
        toast.success("Tópico creado.");
        setPage(1);
        await refresh({ page: 1 });
        resetToNew();
      } else {
        const res = await updateTopico(selected!.id, { codigo: c, descripcion: d, estado });
        toast.success("Cambios guardados.");
        await refresh();
        loadForEdit(res.data);
      }
    } catch (e) {
      toast.error(getApiErrorMessage(e, "No se pudo guardar el tópico."));
    } finally {
      setSaving(false);
    }
  ...
  const onDeactivateConfirmed = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await deactivateTopico(selected.id);
      setConfirmDeactivateOpen(false);
      toast.success("Tópico desactivado.");
      await refresh();
      loadForEdit(res.data);
    } catch (e) {
      setConfirmDeactivateOpen(false);
      toast.error(getApiErrorMessage(e, "No se pudo desactivar el tópico."));
    } finally {
      setSaving(false);
    }

...
import { useDocumentoAtencion } from "../documentoAtencion/hooks/useDocumentoAtencion";
...
export default function DocumentoAtencionPage() {
  const vm = useDocumentoAtencion();
  const isLgUp = useIsLgUp();
  const formRef = React.useRef<HTMLDivElement | null>(null);

  useFicherosRealtimeRefresh(vm, ["documento_atencion"]);
  ...
  return (
    <div className="flex w-full flex-col gap-4 lg:min-h-0 lg:flex-1 lg:overflow-hidden lg:gap-2">
      ...
      <CrudSplitLayout formWidth="480px" rightRef={formRef} left={<>
        ...
        entityLabel="documento de atención"
        mode={vm.mode}
      ...
      <ConfirmDialog
        ...
        title="Desactivar documento de atención"
        description={vm.selected ? `¿Deseas desactivar "${vm.selected.codigo} - ${vm.selected.descripcion}"?` : "Selecciona un documento de atención."}

...
import type { ParamOption, PaginatedResponse, RecordStatus, StatusFilter } from "../../types/paramOption.types";
import {
  createTipoEmergencia,
  deactivateTipoEmergencia,
  getNextTipoEmergenciaCodigo,
  listTipoEmergencia,
  updateTipoEmergencia,
} from "../../services/tipoEmergencia.service";
...
export function useTipoEmergencia() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getNextTipoEmergenciaCodigo();
        if (!alive) return;
  ...
  const refresh = useCallback(async (next?: { page?: number; perPage?: number }) => {
    ...
    try {
      const res = await listTipoEmergencia({
        page: targetPage,
      ...
      const msg = getApiErrorMessage(e, "No se pudo cargar la lista de tipos de emergencia.");
  ...
  const onSave = useCallback(async () => {
    ...
    if (mode === "edit" && !selected) { toast.error("Selecciona un tipo de emergencia para editar."); return; }
    if (!isDirty) { toast.error("No hay cambios para guardar."); return; }
    if (saving) return;
    setSaving(true);
    try {
      if (mode === "new") {
        await createTipoEmergencia({ codigo: c, descripcion: d, estado });
        toast.success("Tipo de emergencia creado.");
        setPage(1);
        await refresh({ page: 1 });
        resetToNew();
      } else {
        const res = await updateTipoEmergencia(selected!.id, { codigo: c, descripcion: d, estado });
        toast.success("Cambios guardados.");
        await refresh();
        loadForEdit(res.data);
      }
    } catch (e) {
      toast.error(getApiErrorMessage(e, "No se pudo guardar el tipo de emergencia."));
    } finally {
      setSaving(false);
    }
  ...
  const onDeactivateConfirmed = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await deactivateTipoEmergencia(selected.id);
      setConfirmDeactivateOpen(false);
      toast.success("Tipo de emergencia desactivado.");
      await refresh();
      loadForEdit(res.data);
    } catch (e) {
      setConfirmDeactivateOpen(false);
      toast.error(getApiErrorMessage(e, "No se pudo desactivar el tipo de emergencia."));
    } finally {
      setSaving(false);
    }

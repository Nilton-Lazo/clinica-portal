...
export function useDocumentoAtencion() {
  ...
  const refresh = useCallback(async (next?: { page?: number; perPage?: number }) => {
    ...
    try {
      const res = await listDocumentoAtencion({
        page: targetPage,
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      if (mode === "new") {
        await createDocumentoAtencion({ codigo: c, descripcion: d, estado });
        toast.success("Documento de atención creado.");
        setPage(1);
        await refresh({ page: 1 });
        resetToNew();
      } else {
        const res = await updateDocumentoAtencion(selected!.id, { codigo: c, descripcion: d, estado });
  ...
  const onDeactivateConfirmed = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await deactivateDocumentoAtencion(selected.id);
      setConfirmDeactivateOpen(false);
      toast.success("Documento de atención desactivado.");
      await refresh();
      loadForEdit(res.data);
    } catch (e) {
      setConfirmDeactivateOpen(false);
